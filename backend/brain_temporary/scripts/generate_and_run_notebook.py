...
def main():
    print("=" * 70)
    print("GENERATING & EXECUTING PARTNER DISCOVERY EXPORTER NOTEBOOK")
    print("=" * 70)
    ...
    def add_markdown(md_text):
        notebook_cells.append({
            "cell_type": "markdown",
            "metadata": {},
            "source": [l + "\n" for l in md_text.strip().splitlines()]
        })

    def add_and_run_code(code_str):
        nonlocal exec_counter, current_cell_outputs
        current_cell_outputs = []
        
        # Clean lines
        lines = [l for l in code_str.strip().splitlines()]
        clean_lines = [l for l in lines if not (l.strip().startswith("%matplotlib") or l.strip().startswith("!"))]
        exec_code = "\n".join(clean_lines)
        ...
        try:
            exec(exec_code, global_ns)
        except Exception as e:
            sys.stdout = old_stdout
            print(f"Error executing cell {exec_counter}: {e}")
            traceback.print_exc()
            raise e
        finally:
            sys.stdout = old_stdout
        ...
        if stdout_text:
            cell_outputs.append({
                "name": "stdout",
                "output_type": "stream",
                "text": [l + "\n" for l in stdout_text.splitlines(True)]
            })
        ...
        print(f"  Executed code cell {exec_counter} successfully.")
        exec_counter += 1

    # =========================================================================
    # 0. Title and Objective
    # =========================================================================
    add_markdown(r"""# Partner Discovery — India as Exporter (EDA, Forecasting & Multi-Criteria Ranking)
...
> The target variables for time-series forecasting are strictly **trade demand / net weight volume (kg)** and **FOB unit export price (USD/kg)** based on official UN Comtrade and DGFT records. We do not claim agricultural crop yield prediction as no farm-level agronomic dataset is present.""")
    ...
    # =========================================================================
    add_markdown("## 1. Imports and Environment Setup\nConfiguring runtime dependencies, reproducible seeds, and printing execution environment specifications.")

    add_and_run_code(r"""# Core numerical and data processing libraries
import os
import sys
import json
import math
...
%matplotlib inline""")
    ...
    add_and_run_code(r"""# Machine learning & modeling
import sklearn
...
print("=" * 60)""")
    ...
    # =========================================================================
    add_markdown("## 2. Locate and Load Project Data\nLocating project exporter files across candidate directories and loading the canonical 26-year trade dataset.")

    add_and_run_code(r"""# Search for canonical exporter dataset across project paths
candidate_paths = [
...
print(f"Loaded Columns     : {df_raw.shape[1]}")""")
    ...
    add_and_run_code(r"""# Display head, tail, and random sample of the raw dataset
...
print(df_raw.dtypes.value_counts())""")
    ...
    add_and_run_code(r"""# Column Inventory Table
col_audit = []
...
display(df_col_audit)""")
    ...
    add_and_run_code(r"""# Duplicate Audit & Business Key Verification
...
if key_dups == 0:
    ...
    add_and_run_code(r"""# Range and Physical Plausibility Checks
range_checks = {
...
display(df_skew)""")
    ...
    add_and_run_code(r"""# Visualization: Raw vs Log1p Distributions for Trade & Economic Variables
...
display(prod_summary.head(10))""")
    ...
    # =========================================================================
    # 9. Destination / Partner EDA
    # =========================================================================
    add_markdown("## 9. Destination Country & Partner Network EDA\nAnalyzing the 53 candidate partners (52 sovereign nations + WLD), identifying top destination corridors across value, volume, and stability.")

    add_and_run_code(r"""# Exclude WLD aggregate for country ranking
...
display(country_summary.sort_values('export_net_weight_kg', ascending=False).head(10).reset_index(drop=True))""")
    ...
    # =========================================================================
    add_markdown("## 10. Market Concentration & HHI Analysis\nMeasuring export destination concentration using Top-5, Top-10 shares and Herfindahl-Hirschman Index (HHI).")

    add_and_run_code(r"""# Overall Destination Market Concentration
...
print(f"Herfindahl-Hirschman Index (HHI): {hhi_index:.1f} (Benchmark: < 1500 indicates competitive, diversified trade)")""")
    ...
    # =========================================================================
    # 11 & 12: Correlation & Bivariate Analysis
    # =========================================================================
    add_markdown("## 11 & 12. Correlation & Bivariate Feature Relationships\nAnalyzing multivariate dependencies between trade flows, macroeconomic scale, applied tariffs, and freight logistics.")

    add_and_run_code(r"""# Correlation Matrix Heatmap
corr_features = [
    'export_value_usd', 'export_net_weight_kg', 'fob_unit_value_usd_per_kg',
...
plt.show()""")
    ...
    add_and_run_code(r"""# Bivariate Relationships
...
plt.show()""")
    ...
    # =========================================================================
    # 13 & 14: Feature Engineering & Leakage Test
    # =========================================================================
    add_markdown("## 13 & 14. Feature Engineering & Strict Leakage Auditing\nBuilding point-in-time lag, rolling momentum, and economic signals with zero future lookahead bias.")

    add_and_run_code(r"""# Feature Engineering Function with Strict Temporal Boundary
...
print(f"Engineered Dataset Shape: {df_features.shape}")""")
    ...
    add_and_run_code(r"""# Programmatic Leakage Audit
...
print("PASSED: Zero forward lookahead leakage detected in lag and rolling calculations.")""")
    ...
    # =========================================================================
    # 15, 16, 17: Sequence Construction & Chronological Partition
    # =========================================================================
    add_markdown(r"""## 15, 16 & 17. Sequence Construction & Chronological Train / Val / Test Split
...
- **Current Context**: 2025""")
    ...
    add_and_run_code(r"""# Sequence Construction (Window Length = 5 Years)
SEQ_LEN = 5
TRAIN_END = 2020
VAL_END = 2022
TEST_END = 2024
...
print(f"Test Holdout         (2023-2024) : X shape = {test_X.shape}, y = {test_y_d.shape}")""")
    ...
    # =========================================================================
    # 18. Baseline Forecast Models
    # =========================================================================
    add_markdown("## 18. Baseline Forecasting Benchmark\nBenchmarking 4 candidate baseline models on the chronological holdout test set (2023–2024).")

    add_and_run_code(r"""# Evaluation metric helpers
...
print("Baseline models fitted successfully.")""")
    ...
    # =========================================================================
    # 19 & 20: GRU Model & Evaluation
    # =========================================================================
    add_markdown("## 19 & 20. Dual-Head PyTorch GRU Model & Multi-Model Evaluation\nBuilding a Shared-Encoder Recurrent Neural Network with separate prediction heads for Demand (kg) and FOB Unit Price ($/kg).")

    add_and_run_code(r"""# Dual-Head GRU PyTorch Architecture
...
print(f"Total Trainable Parameters: {total_params:,}")""")
    ...
    add_and_run_code(r"""# Training Loop with Early Stopping
...
plt.show()""")
    ...
    add_and_run_code(r"""# Comprehensive Model Benchmark Comparison Table
models_dict = {
...
display(df_bench)""")
    ...
    # =========================================================================
    add_markdown("## 21. Forecast Visualization\nVisualizing actual vs predicted demand and price trajectories across sample destination corridors.")

    add_and_run_code(r"""# Actual vs Predicted Plot for Top Partner Corridors
...
plt.show()""")
    ...
    # =========================================================================
    # 22, 23 & 24: Opportunity Ranking, Quantity Simulation & Final Ranking
    # =========================================================================
    add_markdown(r"""## 22, 23 & 24. Opportunity Ranking, Quantity-Fit Simulation & Final Recommendations
Executing the end-to-end recommendation workflow for:
> **"I want to export 1,000 kg of basil seeds"** (HS6 `120999`).""")

    add_and_run_code(r"""# Multi-Criteria Opportunity Ranking with Quantity-Fit and Risk Integration
import sys
from pathlib import Path
...
display(df_final_rank)""")
    ...
    # =========================================================================
    # 25 & 26: Country Detail & Explainability
    # =========================================================================
    add_markdown("## 25 & 26. Country Deep-Dive Detail & Grounded Explainability\nDisplaying granular pros, cons, compliance clearance, and financial projections for top-ranked destination.")

    add_and_run_code(r"""# Deep-Dive Evidence Card for #1 Ranked Destination
...
print("=" * 70)""")
    ...
    # =========================================================================
    # 27 & 28: Save Artifacts & Model Reload Test
    # =========================================================================
    add_markdown("## 27 & 28. Artifact Persistence & Mandatory Model Reload Test\nSaving model checkpoints, scalers, metadata, and verifying independent reloading and inference parity.")

    add_and_run_code(r"""# Save Artifacts
...
print("Artifacts saved successfully to models/ and data/processed/.")""")
    ...
    add_and_run_code(r"""# Mandatory Model Reload Test
...
print("SUCCESS: Reloaded model functions independently with zero loss of numerical precision.")""")
    ...
    # =========================================================================
    add_markdown(r"""## 29. Final Notebook Quality Summary
...
| **9. Model Persistence** | Verified | Saved PyTorch weights, scalers, and passed reload verification test |""")
    ...
    # Save to primary and mirror locations
    ...
    print("=" * 70)
    print(f"SUCCESS: Complete executed analytical notebook saved to:")
    print(f"  1. {nb_path1}")
    print(f"  2. {nb_path2}")
    print("=" * 70)
