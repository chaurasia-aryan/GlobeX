...
from src.compliance.entity_screening import ScreeningDecision, screen_entity, screen_transaction_parties
...
from src.services.rag_synthesizer import synthesize_answer, SynthesisResult
...
class ComplianceRequest(BaseModel):
    ...
    skip_synthesis: bool = Field(default=False, description="Skip the local-LLM synthesis pass (saves a ~10-15s Ollama round trip) when the caller already gets a synthesized answer elsewhere, e.g. from /api/v1/rag/query in the same request chain")
...
class ComplianceResponse(BaseModel):
    ...
    synthesized_answer: Optional[str] = Field(default=None, description="LLM-generated answer grounded in retrieved_evidence, citing passage sources; null if the local LLM was unavailable")
    synthesis_available: bool = Field(default=False, description="True only if synthesized_answer was actually generated by the local LLM, not fabricated or templated")
    synthesis_model: Optional[str] = Field(default=None, description="Ollama model that produced synthesized_answer")
    synthesis_unavailable_reason: Optional[str] = Field(default=None, description="Why synthesis was skipped, when synthesis_available is False")
...
def rag_query(req: RAGQueryRequest) -> Dict[str, Any]:
    ...
    synthesis = synthesize_answer(req.query, passages)
    ...
    return {
        ...
        "synthesis_available": synthesis.synthesized,
        "synthesis_model": synthesis.model,
        "synthesis_unavailable_reason": synthesis.reason,
    }
...
def rag_analyze(req: ComplianceRequest) -> Dict[str, Any]:
    ...
    if req.skip_synthesis:
        synthesis = SynthesisResult(answer=None, synthesized=False, reason="Synthesis skipped by caller request (skip_synthesis=true).")
    else:
        synthesis = synthesize_answer(
            f"What are the tariff, non-tariff, and documentation requirements for exporting HS6 {req.hs6} from {origin} to {dest}?",
            passages,
        )
...
class SanctionsScreenRequest(BaseModel):
    ...
    beneficial_owners: Optional[List[Dict[str, Any]]] = Field(
        default=None,
        description="[{name, pct_ownership}] — screened against the exporter under the OFAC 50% Rule; omit if unknown",
    )
...
def sanctions_screen(req: SanctionsScreenRequest) -> Dict[str, Any]:
    ...
    result = screen_transaction_parties(parties)

    if req.beneficial_owners and req.exporter_name:
        # screen_transaction_parties() only screens names, not ownership —
        # re-screen the exporter with UBO data for the OFAC 50% Rule check
        # (same composition transaction_gate() uses).
        owner_check = screen_entity(req.exporter_name, beneficial_owners=req.beneficial_owners)
        result["per_role"]["exporter_with_ownership"] = owner_check
        if owner_check["decision"] == ScreeningDecision.MATCH_REQUIRES_RESTRICTION:
            result["overall_decision"] = ScreeningDecision.MATCH_REQUIRES_RESTRICTION
